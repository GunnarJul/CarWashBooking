...
namespace CarWashBooking.Logging
{
    public static class FileLoggerExtensions
    {
        ...
        public static ILoggingBuilder AddFile(this ILoggingBuilder loggerBuilder, string filePath, LogLevel minimumLevel = LogLevel.Information)
        {
            ...
            loggerBuilder.AddProvider(new FileLoggerProvider((category, logLevel) => (logLevel >= minimumLevel && logLevel !=  LogLevel.Error ), filePath));

            return loggerBuilder;
        }

        public static ILoggingBuilder AddFileError(this ILoggingBuilder loggerBuilder, string filePath )
        {
            if (String.IsNullOrEmpty(filePath)) throw new ArgumentNullException(nameof(filePath));
            ...
            loggerBuilder.AddProvider(new FileLoggerProvider((category, logLevel) => (logLevel >= LogLevel.Error), filePath));
